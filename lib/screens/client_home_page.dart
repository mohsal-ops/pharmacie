...
class _ClientHomePageState extends State<ClientHomePage> {
  ...
  Future<void> loadLocation() async {
    // Returns null if permission denied instead of crashing
    final Position? pos = await LocationService.getUserLocation();

    if (pos == null) {
      // Permission denied — show message and use a default location
      // so the app still works (Algiers center as fallback)
      setState(() {
        userLocation = const LatLng(36.7372, 3.0865);
        markers.add(
          Marker(
  ...
  Future<void> searchMedicine() async {
    ...
    try {
      ...
      for (var pharmacyDoc in pharmaciesSnap.docs) {
        ...
        final medicines = pharmacyData['medicines'] as List<dynamic>? ?? [];
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'Rechercher un médicament (ex : paracétamol)',
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const Expanded(
                    child: Text(
                      'Localisation désactivée — position par défaut utilisée.',
...
class _MedicineResultCard extends StatelessWidget {
  ...
  const _MedicineResultCard({required this.result, required this.onDirections});
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        medAvailable ? 'In stock' : 'Out of stock',
                        style: TextStyle(
                          fontSize: 13,
                          color: medAvailable
                              ? const Color(0xFF0F9D58)
              ...
              children: [
                    ...
                    Expanded(
                      child: Text(
                        result['pharmacyAddress'].isNotEmpty
                            ...
                            : '${distance.toStringAsFixed(2)} km de distance',
                        style: const TextStyle(
                          color: Colors.grey,
                          fontSize: 13,
                        ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Obtenir un itinéraire'),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: const Color(0xFF0F9D58),
                    ...
                    onPressed: () => onDirections(result['lat'], result['lng']),
                  ),
                ),
              ],
            ),
...
class _PharmacyCard extends StatelessWidget {
  ...
  final Function(double, double) onDirections;
  const _PharmacyCard({required this.pharmacy, required this.onDirections});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.local_pharmacy, color: Color(0xFF0F9D58)),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: () => onDirections(pharmacy['lat'], pharmacy['lng']),
            ),
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Text(
        ...
        style: TextStyle(
          fontSize: 11,
          fontWeight: FontWeight.bold,
          color: isOpen ? const Color(0xFF0F9D58) : const Color(0xFFE53935),
        ),
      ),
    );
  }
